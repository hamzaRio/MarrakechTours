...
export default function EssaouiraGallery({ open, onClose }: EssaouiraGalleryProps) {
  const [currentImageIndex, setCurrentImageIndex] = useState(0);

  const nextImage = () => {
    setCurrentImageIndex((prevIndex) => 
      prevIndex === essaouiraImages.length - 1 ? 0 : prevIndex + 1
    );
  };

  const prevImage = () => {
    setCurrentImageIndex((prevIndex) => 
      prevIndex === 0 ? essaouiraImages.length - 1 : prevIndex - 1
    );
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-3xl p-0 overflow-hidden bg-transparent border-none">
        <DialogTitle className="sr-only">
          <VisuallyHidden>Essaouira Gallery</VisuallyHidden>
        </DialogTitle>
        ...
        <div className="relative w-full h-full">
          ...
          <div className="relative w-full h-full">
            <img 
              src={essaouiraImages[currentImageIndex]} 
              alt={`Essaouira Day Trip image ${currentImageIndex + 1}`}
            ...
            <div className="absolute inset-x-0 bottom-0 flex justify-between p-4 bg-gradient-to-t from-black/60 to-transparent">
              <p className="text-white text-sm">
                {currentImageIndex + 1} / {essaouiraImages.length}
              </p>
            </div>
...
export function EssaouiraThumbnailGallery() {
  const [galleryOpen, setGalleryOpen] = useState(false);

  // Show only the first 4 images as thumbnails
  const thumbnails = essaouiraImages.slice(0, 4);

  return (
    <>
      <div className="mt-6">
        <h3 className="text-xl font-medium text-gray-800 mb-3">Essaouira Gallery</h3>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {thumbnails.map((image, index) => (
            ...
            >
              <img 
                src={image} 
                alt={`Essaouira Day Trip thumbnail ${index + 1}`}
                className="w-full h-full object-cover rounded-md"
              />
              {index === 3 && essaouiraImages.length > 4 && (
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center rounded-md">
                  <p className="text-white font-medium">+{essaouiraImages.length - 4} more</p>
                </div>
              )}
            </div>
          ))}
      ...
      <EssaouiraGallery open={galleryOpen} onClose={() => setGalleryOpen(false)} />
    </>
  );
}